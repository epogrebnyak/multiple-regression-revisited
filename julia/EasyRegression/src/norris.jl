...
# evaluate model
norris = evaluate_ols(Observation(df.x, df.y), intercept=true)

approx(a, b, d)::Bool = (round(a, digits=d) == round(b, digits=d))
@assert approx(reference.b0, norris.beta[1], 12)
@assert approx(reference.b1, norris.beta[2], 12)
@assert approx(reference.r2, r2(norris), 15)